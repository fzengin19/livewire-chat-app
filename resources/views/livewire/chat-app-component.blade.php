<div
    ...
    <div class="w-[400px] lg:min-w-[25%] max-w-[100%] lg:max-w-[25%]  overflow-y-auto hidden lg:block my-0 p-0 border-r items-center flex flex-col border-gray-700 h-[600px]"
        ...
        <div class="h-[60px] sticky top-0  flex items-center m-0 p-0 w-full bg-slate-800 border-b border-gray-700">
            <div class="flex items-center w-full p-3 border-gray-700">
                ...
                <div>
                    <h2 class="font-semibold text-gray-300">{{ auth()->user()->name . ' id:'. Auth::id() }}</h2>
                </div>
            </div>
        </div>
    ...
    @if (isset($activeChat))
    {{-- Active Chat Section --}}
    <div class="w-full my-0 p-0 h-[600px] lg:max-w-[75%]">
        ...
        <div class="border-b pt-2 h-[480px] border-gray-700 overflow-x-auto" id="chatContainer">
            ...
            <div
                ...
                @if ($key === count($activeChat->messages) - 1 || $message->user_id !== $activeChat->messages[$key +
                1]->user_id)
...
<script>
    ...
    document.addEventListener('livewire:initialized', () => {
       @this.on('messageReceived', (event) => {
        scrollSmoothlyToBottom('chatContainer');
       });
    });
</script>